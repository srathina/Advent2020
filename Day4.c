...
void d4part1(void)
{
    ...
    // printf("%s\n", passport[0]);
    // printf("%s\n", fields[0]);
    // printf("%s\n", strstr(passport[0], fields[0]));
    // printf("%d\n", sizeof(fields) / sizeof(fields[0]));
    // printf("%d\n", sizeof(passport) / sizeof(passport[0]));
    ...
    while (i < loop_size)
    {
        for (j = 0; j < (sizeof(fields) / sizeof(fields[0])) - 1; j++)
        {
            if (strstr(passport[i], fields[j]) != NULL)
            {
                valid_count++;
            }
        }
...
void d4part2(void)
{
    ...
    while (i < loop_size)
    {
        for (j = 0; j < (sizeof(fields) / sizeof(fields[0])) - 1; j++)
        {
            result = strstr(passport[i], fields[j]);
